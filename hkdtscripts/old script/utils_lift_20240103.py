...
def StopF_Caculation(paras,seq): # 确定停止的楼层（停止状态）
...
def Traffic_Caculation(paras,seq): # 确定停止的楼层（轿厢行驶状态）
    ...
    if np.isnan(paras['Floor_height']).all() or np.isnan(seq['Distance']).all(): 
        ...
        Depart_F = Arrive_F = 'NAN/F'
        Fs_travelled = F_Travelled = Dir_Travelled = position_dev = np.nan
    else:
        ...
        Depart_F = paras['Floor_level'][Depart_index]  # start floor of this cycle
        Arrive_F = paras['Floor_level'][Arrive_index]  # end floor of this cycle   
        position_dev = np.nanmin(np.abs(seq['Distance'].iloc[-10:].median() - np.array(paras['Floor_height']))) # stop position deviation from the desired floor
        Fs_travelled = Arrive_index - Depart_index # the floors the elevator travels, + upward, - downward
    ...
    return mileage,Depart_F,Arrive_F,Fs_travelled,F_Travelled,Dir_Travelled,position_dev
...
def Stats_PerCarSeg_AC(seq, paras, event_list): # Statistic information for each car segment/cycle (AC type)
    ...
    mileage,Depart_F,Arrive_F,Fs_travelled,F_Travelled,Dir_Travelled,position_dev = Traffic_Caculation(paras,seq)  
    ...
    if 0.1 < Speed_peak < 0.5 and Arrive_F != 'NAN/F':  # 判断是否为慢车检修模式
    ...
    if mileage < 0.2 and Arrive_F != 'NAN/F' and Resv3I_run > paras['thres_resv3']: # (1)没移动；（2）Arrive_F 有值; (3) Resv3 是处于电梯运行的level
...
def Stats_PerDoorSeg(seq, paras, event_list): # Statistic information for each car segment/cycle
    ...
    if paras['DoorOpenLong_FLAG'] == 1 and DoorOpen_Duration > paras['thres_DoorOpenDuration'] and Stop_F != 'G/F': 
...
def SafetyTrip(dataset,paras): 
    ...
    if len(seq_trip)>=120:  
        # if np.isnan(paras['Floor_height']).all() or np.isnan(seq_trip['Distance']).all():        
        #     Stop_F=np.nan
        # else:
        #     Stop_F = paras['Floor_level'][np.nanargmin(np.abs(seq_trip['Distance'].iloc[-1] - np.array(paras['Floor_height'])))] # STOP floor of this cycle
...
def SafetyTrip2(dataset,paras): # safety 和 door 同时满足某个条件
    ...
    if (len(seq_trip1)>=120) & (len(seq_trip2)>=120):
        ...
        # if np.isnan(paras['Floor_height']).all() or np.isnan(seq_trip1['Distance']).all():        
        #     Stop_F=np.nan
        # else:
        #     Stop_F = paras['Floor_level'][np.nanargmin(np.abs(seq_trip1['Distance'].iloc[-1] - np.array(paras['Floor_height'])))] # STOP floor of this cycle
...
def SafetyTrip3(dataset,paras): # safety 连续trip 大于3s (# update on 2023-3-20)
    ...
    if len(TripSeg_list)>0: 
        ...
        # if np.isnan(paras['Floor_height']).all() or np.isnan(dataset['Distance']).all():        
        #     Stop_F=np.nan
        # else:
        #     Stop_F = paras['Floor_level'][np.nanargmin(np.abs(dataset['Distance'].iloc[-1] - np.array(paras['Floor_height'])))] # STOP floor of this cycle
