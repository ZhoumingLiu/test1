...
# def LPF(seq,k):
#     i = 0
#     seq_smooth = np.array([0]*len(seq))
#     N = len(seq)
#     while i < (N-1):
#         seq_smooth[i+1] = k*seq[i+1] + (1-k)*seq_smooth[i] 
...
def Data_segment(dataset):
    ...
    # dataset['Motor_KF'] = LPF(dataset['Motor'],0.1)
    # # dataset['Motor_KF'] = dataset['Motor'].apply(lambda x: np.nan if dataset['Motor'].isnull().all() else LPF(x,0.1))        

    # dataset['Brake_KF'] = LPF(dataset['Brake'],0.1)
    # dataset['Safety_KF'] = LPF(dataset['Safety'],0.1)
    # dataset['Door_KF'] = LPF(dataset['Door'],0.1)
    # # dataset['Resv1_KF'] = dataset['Resv-1'].apply(lambda x: np.nan if dataset['Resv-1'].isnull().all() else LPF(x,0.1))        
    # dataset['Resv1_KF'] = LPF(dataset['Resv-1'],0.1)
    # dataset['Resv2_KF'] = LPF(dataset['Resv-2'],0.1)
    # dataset['Resv3_KF'] = LPF(dataset['Resv-3'],0.1)
    # dataset['Dist_KF'] = LPF(dataset['Distance'],0.1)
    # dataset['Vel_KF'] = LPF(dataset['Velocity'],0.1)
...
def Stats_PerCarSeg(seq, paras, event_list): # Statistic information for each car segment/cycle
    ...
    BrakeI_steady = seq['Brake_KF'].iloc[(round(len(seq)/2)-15):(round(len(seq)/2)+15)].mean() #average brake steady current when car runs    
    ...
    # Inspection     
    if 0.1 < Speed_peak < 0.5:  # 判断是否为慢车检修模式
        log_text = {
            "time": str(end_time),
            "status ID": 4,
...
def Stats_PerDoorSeg(seq, paras, event_list): # Statistic information for each car segment/cycle
    ...
    if DoorOpen_Duration > 30 and duration < 55: # 开门时间过长
        log_text = {
            "time": str(end_time),
            "status ID": 3.1,
            "event": "Door anomaly",
...
def LSTMFCN_Motor(seq,end_time,event_list):
    with tf.Graph().as_default():
        with tf.Session() as sess:
            K.set_session(sess)
            model = model_from_yaml(open('/app/hkdtscripts/devpy/inspection_model.yaml').read(), custom_objects={"AttentionLSTM": AttentionLSTM})
            model.load_weights('/app/hkdtscripts/devpy/inspection_model_weights.h5', by_name=False)
...
def LSTMFCN_Door(seq,end_time,event_list):
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            model = model_from_yaml(open('/app/hkdtscripts/devpy/model.yaml').read(), custom_objects={"AttentionLSTM": AttentionLSTM})
            model.load_weights('/app/hkdtscripts/devpy/my_model_weights.h5', by_name=False)
...
def LSTMAE_Brake(criterion,brake_result,brake_list,CarSeg_list,BrakeError_max,event_list):
    ...
    f_brake = open('/app/hkdtscripts/devpy/Brake_LSTM_AE.pt', 'rb')
    # f_motor = open('/app/hkdtscripts/devpy/Motor_LSTM_AE.pt', 'rb')
