# -*- coding: utf-8 -*-
"""
Created on Tue Feb 20 11:53:46 2024
...
meanIds = {'962712300012':'Motor','962712300013':'Brake','962712300014':'Safety',
           '962712300015':'Door','962712300016':'Resv-1','962712300017':'Resv-2',
           '962712300018':'Resv-3','962712300022':'FFT-Motor Freq','962712300021':'FFT-Motor Mag',
           '962712300025':'FFT-Resv1 Freq','962712300024':'FFT-Resv1 Mag',
           '962712300028':'FFT-Resv2 Freq','962712300027':'FFT-Resv2 Mag','962712300019':'Distance',
           '962712315034':'SAD','962712315033':'E','962712315032':'P2P','962712315031':'Mean',
           '962712315030':'Var','962712315029':'Skewness','962712315028':'Kurtosis','962712315027':'MAD',
           '962712315026':'CF','962712315025':'SF','962712315024':'IF'}

mmumeanIds = {'962715451075':'floor','962715451077':'velocity','962715451069':'mileage',
              '962715451087':'openCloseDoorNum','962715451091':'vibration_x',
              '962715451092':'vibration_y','962715451093':'vibration_z','962715451081':'door',
              '962715451086':'workMode','962715451076':'motion','962715451079':'cumulativeRunNum' }
...
def getPoints(stationId,devId,means):
    meanSql=""
    for index in range(len(means)):
        if index==0:
            meanSql+=means[index]
            ...
            meanSql+=","+means[index]
    sql="SELECT p.C0110_POINT_ID,p.C0110_FARM_TERM_NO,p.C0029_MEASTYPEID FROM sys_m_point p,sys_m_device d WHERE p.C0003_DEVID=d.C0003_DEVID and p.C0002_STATION_NO="+'%d'%stationId+" and d.c0003_parent_id="+devId+"  and p.C0029_MEASTYPEID in ("+meanSql+") order by p.sort"
    # print(sql)
    return  MySqlUtil.getData(sql)
...
def Load_Build_DataFrame(dateStr, paras, Lift_ID): # 从数据库抓取和组装数据
    ...
    frameData['Time'] = timeStrs
    allmeans=[]
    for mean in meanIds.keys():
        allmeans.append(mean)
    # 获取电梯设备的电流传感器、距离传感器设备的测点
    tempPoints = getPoints(data[2], data[0], allmeans)
    for point in tempPoints:
            points.append(point)
    ...
    for index in range(len(points)):
        ...
        for time in times:
            if (time not in mondata):
                ...
                framKeyData.append(mondata[time]['value'])
        title=None
        if str(point[2]) in meanIds.keys():
            title=meanIds[str(point[2])]
        elif str(point[2]) in mmumeanIds.keys():
            title = mmumeanIds[str(point[2])]

        if title is not None:
            frameData[title] = framKeyData        
    # 组装为pandas 数据对象
...
def EgPlot_SingleFeature(df,feature,deviceID, fig_L, fig_W, fontsize, linewidth):
    ...
    lns0 = ax1.plot(seq, linestyle='-', 
                    linewidth=linewidth, color='#0c0c0d', label='RMS')
...
#    ax2.set_ylim([0,50])
    ...
    plt.title(deviceID + '|' + 'Door Motor Current ' + 'RMS' + ' Sequence')

    plt.show()
    
    
    
def EgPlot_RawFeature_Vibration(sequence, deviceID, fig_L, fig_W, fontsize, linewidth): # 画图
    sns.set(rc={'figure.figsize':(fig_L, fig_W)})
    sns.set(font_scale=fontsize) 
...
#    ax1.set_ylim([0,2.0])
    ...
    ax1.legend(lns, labs, bbox_to_anchor=(0.5, 1.14), loc='upper center', fancybox=True, 
                framealpha=1, shadow=True, ncol = 4)
    plt.title(deviceID + '|' + str(sequence.index[0]) + '---' + str(sequence.index[-1]),pad=39)
    # plt.show()
    ...
    # fig, ax3 = plt.subplot(3,1,2)
    color = 'tab:blue'
    lns7 = ax3.plot(sequence['vibration_x'], linestyle='-', 
                    linewidth=linewidth, color='#1F77B4', label='vibration_x')
    lns8 = ax3.plot(sequence['vibration_y'], linestyle='-', 
                    linewidth=linewidth, color='tab:red', label='vibration_y')
    lns9 = ax3.plot(sequence['vibration_z'], linestyle='-', 
                    linewidth=linewidth, color='#0c0c0d', label='vibration_z')    
    ax3.tick_params(axis='y', labelcolor=color)
    ax3.set_ylabel('Vibration (m^2/s)', color=color)
    ...
    # ax3.legend(lns, labs, loc='upper center', fancybox=True, 
    #            framealpha=1, shadow=True, ncol = 3)

    ax3.legend(lns, labs, bbox_to_anchor=(0.5, 1.14), loc='upper center', fancybox=True, 
                framealpha=1, shadow=True, ncol = 3)    

    plt.show()      
    
    
def EgPlot_NewFeatures(df):    
    
    # 列出你想绘图的特征列名
    features = ['Door', 'SAD', 'E', 'P2P', 'Mean', 'Var', 
                'Skewness', 'Kurtosis', 'MAD', 'CF', 'SF', 'IF']
    
    # 设置图形的全局字体大小和线宽，以增强清晰度
    plt.rcParams.update({
        'font.size': 14,
        'axes.labelweight': 'bold',
        'axes.titleweight': 'bold',
        'axes.labelsize': 'large',  # 加大轴标签字体大小
        'xtick.labelsize': 'medium',  # 加大X轴刻度字体大小
        'ytick.labelsize': 'medium'  # 加大Y轴刻度字体大小
    })
    
    # 创建一个图形和一组子图（6行2列），并调整子图的宽度
    fig, axes = plt.subplots(nrows=6, ncols=2, figsize=(20, 30), sharex=True)
    axes = axes.flatten()
    
    for i, feature in enumerate(features):
        sns.lineplot(x='Time', y=feature, data=df, ax=axes[i])
...
def feature_unit_convert(dataset, Coef): 

    
    dataset['SAD'] = dataset['SAD'] * Coef
    dataset['E'] = dataset['E'] * 10000 * Coef ** 2
    dataset['P2P'] = dataset['SAD'] * Coef
    dataset['Mean'] = dataset['Mean'] * Coef
    dataset['Var'] = dataset['Var'] * 100 * Coef ** 2
    dataset['Skewness'] = (dataset['Skewness'] - 10000) * 0.001
    dataset['Kurtosis'] = dataset['Kurtosis'] * 0.01
    dataset['MAD'] = dataset['MAD'] * Coef
    dataset['CF'] = dataset['CF'] * 0.01
    dataset['SF'] = dataset['SF'] * 0.001
    dataset['IF'] = dataset['IF'] * 0.01
