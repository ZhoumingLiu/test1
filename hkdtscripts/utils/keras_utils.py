...
class Metrics(Callback):
  def on_epoch_end(self, epoch, logs={}):
    _, _, X_test, y_test, is_timeseries = load_dataset_at(50, normalize_timeseries=False)
    pred = self.model.predict(X_test, batch_size=128)
    pred_label = np.argmax(pred, axis=1)  # the predicted label, one column
    y_test = np.load('/home/gyj/Desktop/MLSTM-FCN/data/rengong_1200_2class_AB017_202103_Door/y_test.npy')
    true_label = y_test[:, -1].astype(int)
...
def train_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
    ...
    callback_list = [model_checkpoint, reduce_lr, metrics]
...
def evaluate_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, batch_size=128, test_data_subset=None,
    ...
    if max_timesteps != MAX_TIMESTEPS_LIST[dataset_id]:
        X_test = X_test[:, :, :MAX_TIMESTEPS_LIST[dataset_id]]
