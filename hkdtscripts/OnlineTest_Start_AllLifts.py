# -*- coding: utf-8 -*-
"""
Created on Tue Apr  5 23:17:19 2022
...
TITLES=['Time','Motor','Brake','Safety','Door','Resv-1','Resv-2','Resv-3','FFT-Motor Freq','FFT-Motor Mag','FFT-Resv1 Freq','FFT-Resv1 Mag','FFT-Resv2 Freq','FFT-Resv2 Mag','Distance','floor','velocity','height','mileage','door','workMode']
...
def calModeData(dateStr):
    ...
    #获取电梯设备列表
    #
    sql="SELECT d.`C0003_DEVID`,d.`C0003_DEVNAME`,d.`C0002_STATION_NO`,d.`C0002_STATION_NAME`,d.`c0003_field31` ,MOD(d.`C0003_DEVID`,%s"%hashn+") modv,d.api_config FROM sys_m_device d WHERE c0003_devtype='dt' and c0003_field31 is not null  AND clean_flag IS NOT NULL AND clean_flag!=4 "
...
#     # sql="SELECT d.`C0003_DEVID`,d.`C0003_DEVNAME`,d.`C0002_STATION_NO`,d.`C0002_STATION_NAME`,d.`c0003_field31`  FROM sys_m_device d WHERE c0003_devtype='dt' and c0003_field31 is not null and C0003_DEVID=655148485457"
...
#     print(result)
#     # 对结果进行解析存储
#     # saveResult(data[2],data[0],result,CarSeg_Stats,DoorSeg_Stats)
#     # print(DoorSeg_Stats)
...
# def saveResult(floorId,liftId,result,CarSeg_Stats,DoorSeg_Stats):
#     #保存result
#     isReset=0
#     if result['last_status'] == 0:
#         isReset=1
...
#             continue
#         tup = (oriStatus['result_id'], event['description'], event['status ID'], event['time'])
#         eventVal.append(tup)
#     insertEvent = "INSERT INTO bs_lift_events(result_id,event_desc,status_id,event_time) VALUES(%s,%s,%s,%s)"
#     if (len(eventVal) > 0):
#         sql = "delete from bs_lift_events where result_id=%s" %oriStatus['result_id'] + " and event_time='"+"%s"%event['time']+"'"

#         MySqlUtil.executSql(sql)
#         MySqlUtil.saveBatchData(insertEvent, eventVal)

#     #保存 CarSeg_Stats
#     carstat = []
#     insertCarseg = "INSERT INTO bs_carseg_stats (lift_id,start_time,end_time,duration,hour,mileage,depart_f,arrive_f,fs_travelled,f_travelled,dir_travelled,motori_start," \
#                    "motori_peak,brakei_steady,brakei_peak,safetyi_run,resv1i_start,resv1i_peak,resv2i_start,resv2i_peak,resv3i_run,speed_peak ) " \
#                    "values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"

#     for carseg in CarSeg_Stats['CarStat_list']:
#        tup=(liftId,carseg['start_time'],carseg['end_time'],carseg['duration'],carseg['hour'],carseg['mileage'],carseg['Depart_F'],carseg['Arrive_F'],carseg['Fs_travelled'],carseg['F_Travelled'],carseg['Dir_Travelled'],
#             carseg['MotorI_start'],carseg['MotorI_peak'],carseg['BrakeI_steady'],carseg['BrakeI_peak'],carseg['SafetyI_run'],carseg['Resv1I_start'],carseg['Resv1I_peak'],
#             carseg['Resv2I_start'],carseg['Resv2I_peak'],carseg['Resv3I_run'],carseg['Speed_peak'])
#        delCarSql = "delete from bs_carseg_stats where lift_id=%s" % liftId + " and end_time='" + "%s" % carseg['end_time'] + "'"
#        MySqlUtil.executSql(delCarSql)
#        carstat.append(tup);
#     if(len(carstat)>0):
#         MySqlUtil.saveBatchData(insertCarseg, carstat)
...
#     doorstat = []
#     insertDoorseg = "INSERT INTO bs_doorseg_stats (lift_id,start_time,end_time,duration,hour,doori_peak,num_door,dooropen_duration)" \
#                    " values (%s,%s,%s,%s,%s,%s,%s,%s)"

#     for doorseg in DoorSeg_Stats['DoorStat_list']:
#         if(doorseg['duration']<=59):
#             tup = (liftId, doorseg['start_time'], doorseg['end_time'], doorseg['duration'], doorseg['hour'],doorseg['DoorI_peak'], doorseg['num_Door'], doorseg['DoorOpen_Duration'])
#             delDoorSql = "delete from bs_doorseg_stats where lift_id=%s" % liftId + " and end_time='" + "%s" % doorseg['end_time'] + "'"
#             MySqlUtil.executSql(delDoorSql)
#             doorstat.append(tup);
#     if (len(doorstat) > 0):
#         MySqlUtil.saveBatchData(insertDoorseg, doorstat)
